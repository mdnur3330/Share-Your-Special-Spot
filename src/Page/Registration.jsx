...
const Registration = () => {
    const {emailandpassregtration, handelUpdateProfile , loginByEmail} = useContext(AuthContext)
    ...
    const handelRegistration = (e)=>{
        ...
        emailandpassregtration(email, password).then(result => {
            console.log(result)
    ...
    <div className="card bg-base-100 shrink-0 w-full max-w-sm">
      <div className="card-body">
        ...
        <form className="fieldset text-2xl" onSubmit={handelRegistration}>
          ...
          <input type="url" name='photo' className='input'placeholder='Photo' />
          <label className="label">Email</label>
          <input type="email" name='email' className="input" placeholder="Email" />
          <label className="label">Password</label>
          <input type="password" name='password' className="input" placeholder="Password" />
          
          <button type='submit' className="btn btn-neutral mt-4 text-xl">Ragistiation</button>
        </form>
...
<p className='p-4'>alreday have an account <Link to={'/login'}><span className='text-indigo-600 font-medium underline'>Sign In</span></Link></p>
